	...
	public LibraryManagement() {
		...
		addBookButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					Database.addBook(booktitleField.getText(), authorField.getText(), genreField.getText(),
							pubDateField.getText(), isbnField.getText(), availableField.isSelected());
		...
		btnAddBorrower.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					Database.addBorrower(nameField.getText(), emailField.getText(), phoneField.getText(),
		...
		btnAddcheckout.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					Database.addCheckout(bookIDField.getText(), borrowerIDField.getText(), checkoutDateField.getText(),
							dueDateField.getText(), returnDateField.getText());
					Database.refreshTables(bookModel, borrowerModel, checkoutModel);
				} catch (SQLException e1) {
		...
		bookTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					...
					if (selectedRow != -1) {
						Object firstCellValue = bookTable.getValueAt(selectedRow, 0);
						deleteBookField.setText(firstCellValue.toString());
					}
				}
			}
		});

		borrowerTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			@Override
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					int selectedRow = borrowerTable.getSelectedRow();
					if (selectedRow != -1) {
						Object firstCellValue = borrowerTable.getValueAt(selectedRow, 0);
						deleteBorrowerField.setText(firstCellValue.toString());
					}
				}
			}
		});
		...
		checkoutTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					...
					if (selectedRow != -1) {
						Object firstCellValue = checkoutTable.getValueAt(selectedRow, 0);
						deletecheckoutField.setText(firstCellValue.toString());
					}
				}
			}
		});
